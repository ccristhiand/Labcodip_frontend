...
import { SistemaClienteExamen } from '../models/sistemacliente.model';


@Injectable({
  providedIn: 'root'
})
export class SistemaExamenService {
  ...
  constructor(
    ...
    private url: string = `${environment.UrlApi3}/sistemaclienteexamen`;
    ...
    Guardar(id: string, model: SistemaClienteExamen){
      if(id==""){
        let urls = `${this.url}`;
